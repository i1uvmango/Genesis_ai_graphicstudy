...
#### 예상 원인들
1. dx_local, dy_local 좌표계 계산이 잘못되어 피처로서의 가치가 없음 (상관관계 0).
2. 모델은 이 무의미한 피처를 무시하고 학습함.
3. 대신 g_ang_vz 등 다른 피처에 의존하여 "평균적인 좌회전"만 학습함. (8자 주행은 좌회전/우회전이 섞여있지만 데이터 불균형이나 초기 조건 때문에 한쪽으로 쏠림)
