...
### UKMAC
* **Target 설정:** 데이터셋에서 $target\_a$(목표 가속도)와 $target\_k$(목표 곡률)를 먼저 계산합니다.
* **최적화(Minimize):** 시뮬레이터에서 가상의 핸들과 엑셀을 조절해 보며, 거기서 발생하는 `pred_a`와 `pred_k`가 목표값($target\_a, k$)과 일치할 때까지 반복합니다.
* **결과:** 이때 찾아낸 핸들과 엑셀 값이 바로 **$s_{opt}, t_{opt}$**가 됩니다.

```python
def objective(x):
    t_cand, s_cand = x # 최적화 변수
    
    # 1. 모델이 예측한 가속도와 곡률
    v_next, yr_next = engine.step_and_measure(t_cand, s_cand)
    pred_a = (v_next - v_curr) / dt
    pred_k = yr_next / max(abs(v_next), 0.5)
    
    # 2. Stage 2 모델로 보정한 최종 물리량 (Physics-Augmented)
    corr_a = pred_a + (delta_env / dt)
    corr_k = pred_k + delta_dyn_k
    
    # 3. 목표 a, k와의 오차 최소화 (UK MAC 최적화 핵심)
    return (corr_a - target_a)**2 + 10.0 * (corr_k - target_k)**2
```
...
* **Loss 매우 낮음:** $(steer\_b, throttle\_b)$와 $(steer\_g, throttle\_g)$의 차이가 매우 작음.
...
* **구조:** 입력(Input) $\rightarrow$ 제어기(Controller) $\rightarrow$ 프로세스(Process) $\rightarrow$ 출력(Output)
* **특징:**
    * 피드백(Feedback)이 없습니다.
...
<video controls src="5.mp4" title="Inference"></video>
