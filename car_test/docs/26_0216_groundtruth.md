...
## 공간 기반 err 계산
* 시간 기반은 genesis 환경에서 차량이 csv 에 기록된 속도와 시간을 100% 지킨다는 가정하에 작동
* **최적화기의 시각**: 최적화 알고리즘(L-BFGS-B)은 현재 내 위치(x, y)에서 목표 점(x_ref, y_ref)까지의 거리를 최소화
* **시간 기반 인덱싱 문제**: 목표점이 먼저 가게 된다면 최적화기는 지름길로 가로질러 가는 법을 계산

### fine tuning (공간기반)
| window | accel_weight | steer_weight | beta_cte | beta_he | 결과 | 비고 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 12 | 1.0 | 1.0 | 50.0 | 10.0 | - | 비교를 위해 시간기반 best랑 같은 가중치 |
