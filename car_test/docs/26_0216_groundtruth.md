...
bicycle model의 전방속도 이므로(`차량의 속도`) 이므로 `v_local`로 `a`를 추출 했고, k를 위치정보를 사용하여 추출
...
| 프로세스 | window | accel_weight | steer_weight | beta_cte | beta_he | 결과 | 비고 |
| :---| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 12 | 5| 1.0 | 1.0 | 50.0 | 10.0 |  ![](../res/0208/opt_ak.png) |
| 12 | 5 | 5.0 | 15.0 | 20.0 | 10.0 | ![](../res/0208/vel_up.png) |
| - | - | - | - | - | - | - | 아래부터는 타이어 슬립이 원인이라 생각해서 핸들의 반응성 10배 높여줌 | 
| 12 | 12 | 1.0 | 50.0 | 10.0 | ![](../res/0216/gt1.png) | - |
| 12 | 12 | 1.0 | 0.1 | 150.0 | 50.0 | 오차 한계점 넘어 중단 | 조향 우선, 경로 벌점 max &rarr; 진동 발생 |
| 12 | 12 | 1.0 | 0.5 | 50.0 | 80.0 | 오차 한계 넘어 중단 | 진동줄이기 위해 횡방향 벌점 낮춤   |
| 1 | 12 | 1.0 | 0.1 | 150.0 | 30.0 | cte, he 모두 오차 폭발 | 단일 스레드 |
| 1 | 5 | 1.0 | 0.1 | 500.0 | 200.0 | 원인: 속도로 추정 | 계산량 낮추고 가중치 대폭 늘려 원인 탐색 |
| 1 | 5 | 0.1 | 0.1 | 500.0 | 200.0 | 리셋 | 속도가중치 1&rarr;0.1 조정 |
...
![alt text](image.png)
* 120 frame
* 슬립에 의해 변하지 않아서 bicycle model에 의한 경로는 일치
  * 빨강은 경로
  * 초록은 차량 경로
  * 파랑은 차량의 슬립한 경로
...
# 문제 발견
>a,k를 동일하게 맞추고 csv를 참고하여 v,yaw,cte,he를 최적화 하고 있었음
하지만 사실 (a,k)는 다른 변수들과 매우 밀접하게 관련되어있음. (a,k)를 고정시키고 최적화 하는게 말이 안돼서 결과가 나오지 않았던 것.
