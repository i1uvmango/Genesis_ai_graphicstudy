...
* 학습 편향이 일어났다고 생각하고 해결해보고자 함(ex: 좌회전 데이터에 편향 &rarr; 스케일, 양 맞춰야함)
...
2. (유력하다고 생각함) "평균적인 좌회전"만 학습함. (8자 주행은 좌회전/우회전이 섞여있지만 데이터 불균형이나 초기 조건 때문에 한쪽으로 쏠림)
   * 다른 데이터(좌/우회전, 직진, 브레이크, 등 다양한 데이터도 포함해서 학습했는데 왜 그러는지는 의문
4. 따라서 dx_local, dy_local이 유의미한 피처가 되도록 고치는 것이 핵심입니다.
