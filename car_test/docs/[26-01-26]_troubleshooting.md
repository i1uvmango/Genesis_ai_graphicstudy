...
* **[문제점]**
    * Blender 데이터가 물리적 노이즈 없는 '레일 주행' 상태여서 $\text{CTE}, \text{HE} \approx 0$인 데이터만 생성됨.
...
* **[해결책: 데이터 증강 및 노이즈 주입]**
    ...
    * **Stage 4 (Input Jittering):** 학습 시 $\text{CTE}, \text{HE}$ 입력값에 미세한 노이즈를 섞어 모델의 유연성(Robustness) 확보.
    * **Trajectory Sampling:** Look-ahead 거리를 가변적으로 설정하여 다양한 곡률에 대응.

#### 좁은 표준편차($\sigma=0.1$)의 함정: "터널 시야"
* 학습 데이터에서 오차($\text{CTE}$)의 표준편차가 0.1 정도로 매우 작다는 것은, 모델이 경로 중심에서 $\pm 0.1m$ 이내에 있는 상황만 수만 번 경험했다는 뜻입니다.
* **학습 영역 (Safe Zone):** $\text{CTE} \in [-0.1, 0.1]$ → 모델이 "어떻게 해야 할지" 잘 아는 구간.
* **미지의 영역 (Danger Zone):** $\text{CTE} > 0.1$ 또는 $\text{CTE} < -0.1$.
    * 제네시스 시뮬레이션에서 물리적 차이로 인해 차가 아주 살짝 밀려 $\text{CTE}=0.15$가 되는 순간, 모델 입장에서는 **태어나서 처음 보는 "외계 환경"**에 처하게 됩니다.

#### 왜 발산(Explosion)으로 이어지는가?
이 현상은 보통 아래와 같은 **악순환(Positive Feedback Loop)**을 그리며 발생합니다.
1. **미세 오차 발생:** 제네시스 엔진 특성상 $\text{CTE}$가 $0.1$을 살짝 넘어감.
2. **모델의 패닉 (Out-of-Distribution):** 경험해본 적 없는 $0.1$ 이상의 입력을 받자, MLP의 활성화 함수가 비정상적으로 높은 값을 출력함.
3. **과도한 조작:** 모델이 핸들을 너무 세게 꺾거나 반대로 꺾어버림.
4. **오차 증폭:** 잘못된 조작으로 $\text{CTE}$가 $0.1$에서 $0.5, 1.0$으로 순식간에 커짐.
...
* **기존 데이터:** $\text{CTE} \sim \mathcal{N}(0, 0.01)$ (매우 좁음, 복원력 없음)
* **증강된 데이터:** $\text{CTE} \sim \mathcal{N}(0, 0.5)$ (의도적으로 0.5m까지 오차 상황을 만듦)
* 표준편차를 키워서 학습시키면, 모델은 $\text{CTE}=0.2$나 $0.3$인 상황에서도 **"아, 이 정도는 핸들을 이만큼 꺾어서 돌아가면 돼"**라는 **복원 규칙(Recovery Rule)**을 가지게 됩니다.
...
* **[해결 및 설계 유의사항]**
    * **Look-ahead(예견 제어) 강화:** 현재 오차뿐만 아니라 미래 오차($\text{la\_CTE}$)를 피처로 넣어 '미리' 핸들을 꺾도록 설계.
    * **Damping 강화:** Stage 3의 목적 함수에서 $\text{HE}$(Heading Error) 페널티를 높여 방향 변화를 억제하거나, Genesis의 kv 게인을 조절하여 물리적 감쇠력을 확보.
...
* **[설계 가이드라인]**
    $$ \mathcal{L} = (a_{gen} - a^{*})^2 + 5(k_{gen} - k^{*})^2 + \beta_1 \text{CTE}^2 + \beta_2 \text{HE}^2 $$
    * 조향(곡률)은 가속도보다 물리적 민감도가 높으므로 통상 5배 이상의 가중치 부여.
    * **$\beta$ 튜닝 순서:** 먼저 물리 번역($a, k$)이 선행된 후, 서서히 경로 페널티($\text{CTE}$)를 올려가며 복원력을 주입해야 함.

---

### 4. Stage 1 & 2의 존재 이유 (Why not meaningless?)
* **[유의 사항]**
    * Stage 1, 2에 $\text{CTE}$가 없는 것은 설계 오류가 아닌 **'도메인 분리'** 전략임.
...
Stage 1과 2의 학습 데이터에 $\text{CTE}$가 없어도 되는 이유는, 이 단계들이 해결하려는 문제가 **"경로를 지키는 법"**이 아니라 **"힘의 크기를 맞추는 법"**이기 때문입니다.
...
* 즉, Stage 1 & 2는 $\text{CTE}$가 $0$인 상태, 즉 **'완벽하게 달리고 있을 때'조차 발생하는 두 엔진 사이의 물리적 괴리($Residual$)를 메우는 기초 공사**입니다.
...
# gt할때 timestep 을 더 잘게 쪼개야함
* state도 더 세분화하게 들어가야함 -> 근데 stage1,2는 ? 이걸로는 안되나?


golden 값은 얼마나 따라가야하는지.. 90% 따라가는 정도
