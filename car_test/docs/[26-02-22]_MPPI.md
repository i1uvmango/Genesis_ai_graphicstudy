...
$$m\dot{v}_x = F_x, \quad m\dot{v}_y = F_y, \quad I_z \dot{r} = M_z$$

| 변수 | 설명 |
| :--- | :--- |
| 타이어 슬립각 | 타이어가 실제 진행 방향과 향하는 방향의 차이 |
| 마찰계수 | 노면과 타이어 간 마찰력 |
| 횡력 포화 | 타이어가 낼 수 있는 횡력의 한계 |
| 언더/오버스티어 | 조향 입력 대비 실제 회전 차이 |
| 하중 이동 | 가감속/선회 시 타이어별 하중 변화 |

> 같은 throttle, steering을 넣어도 실제 생성되는 $a, k$가 다를 수 있음.
> $k = \frac{r}{v}$이고 $r = \frac{M_z}{I_z}$이므로, yaw rate는 모멘트에 의해 결정됨.
