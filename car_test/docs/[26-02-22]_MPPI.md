...
> "수많은 미래를 실행해보고, 그중 잘된 것들을 가중평균해서 지금 뭘 할지 정하는" 제어 방법.


 왜 MPPI를 쓰나?

  장점
  - 차 물리가 아무리 복잡/비선형이어도 OK — 미분 없이 "앞으로 굴려보기"만 되면 됨.
  - GPU로 2048개 미래를 한 방에 굴리니 실시간 가능.
  - cost 함수만 바꾸면 목적(빠르게/안전하게/부드럽게)을 자유롭게 바꿈.

  단점
  - 본질적으로 랜덤 샘플링 → 운이 나쁘면 그 스텝 계획이 흔들림 (그래서 corr_alpha(next step warmstart)로 노이즈를 시간축으로 부드럽게 함).
  - 샘플이 적거나 noise가 작으면 좋은 수를 아예 안 시도해서 놓침.
  - cost 가중치 튜닝이 성능을 좌우 → grid search의 필요성


### from MPC to MPPI : Method shift
...
| - | - | - |
| ![](../res_wjdaksry/0216/stage2_1.png) | ![](../res_wjdaksry/0216/stage2_2.png) | ![](../res_wjdaksry/0216/stage2_3.png)| 
...
#### 결과
![](../res_wjdaksry/0216/stage2_3.png)
