...
1. **Step 1: Spacing 밀도 조율 (`0.1` → `0.25`)**
   * **내용**: 격자 칸의 크기를 키워 실제 들어오는 vertex와 격자의 비율(밀도)을 맞췄습니다.
   * **효과**: 빈 공간(NaN)의 광범위한 발생을 억제하여 지형 격자의 단단한 뼈대를 우선 확보합니다.

2. **Step 2: Cubic Interpolation (3차 다항 보간 파이프라인)**
   * **내용**: 그럼에도 불구하고 남겨지는 NaN 구멍 구간을 $Z$축 주변 데이터값을 참조해 부드러운 3차 곡선(C1 연속)으로 채워 넣었습니다.
...
* **$\phi$ (Roll, 좌우 기울기)**
  ...
  * **원리**: 좌우로 기울어짐에 비례하여 페널티($\phi^2$)를 부여해, 오직 0 도 수평 상태를 유지하도록 억제합니다.
