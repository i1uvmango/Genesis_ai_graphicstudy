...
| 방식 | 결과 | 비고 |
|------|------|------|
| Spring force (Step 4) | X | 관통 후 폭발적 force → 튕김 |
| Constraint 근사 (Step 6) | O | z_std=0.000, 즉각 수렴 |
| Constraint + Traction (Step 7) | △ | 수평력 디버깅 중 |
...
#### read_and_clamp() 핵심 코드

```python
comp = TIRE_RADIUS - hit_dist   # 관통 깊이 계산

if comp > 0.001:
    saved_dv = car.get_dofs_velocity(all_dofs).clone()  # ① 속도 저장
    car.set_pos([[x, y, pos_z + max_comp]])              # ② 위로 밀어올림
    saved_dv[0, BASE_Z_DOF] = 0.0                        # z 하향 속도 제거
    car.set_dofs_velocity(saved_dv, all_dofs)            # ③ 속도 복원
```

#### PhysX constraint와 비교
...
- O: Genesis Lidar API 작동 확인 (GridPattern -z)
- O: Genesis Lidar self-ignore 동작 확인
