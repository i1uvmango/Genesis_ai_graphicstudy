...
```
GJK (Gilbert-Johnson-Keerthi): 두 convex 물체의 최단거리/충돌여부를 Minkowski 차집합으로 판정 (관통 시 depth는 별도)
MPR (Minkowski Portal Refinement): GJK 보완 — 관통 상태에서도 penetration depth와 normal을 한 번에 산출, GPU 친화적
...
* Ray 4개 × BVH 교차 = 극히 가벼운 연산
  * 2000개 env도 GPU 배치로 처리 가능
  * 충돌 계산 비용이 1/10 이하로 줄어듦
  * 같은 연산 예산으로 substep을 더 늘릴 수 있음


### ray test(Genesis Lidar)
* Genesis Lidar는 attach된 entity를 self-ignore 하도록 설계됨
  * Ray가 자기 바퀴를 통과하는 게 버그가 아니라 의도된 동작
  * 덕분에 wheel collision 비활성화 없이도 terrain까지 정확히 측정 가능
...
```
현재 서스펜션 길이 = hit_distance - wheel_radius
compression       = restLength - currentLength
                  = restLength - (hit_distance - wheel_radius)

restLength = 서스펜션이 완전히 늘어난 길이 (최대값)

suspensionForce = stiffness × compression
                + damping   × (prevLength - currentLength) / dt
```

### CARLA 비대칭 damper

- 압축 시 (낙하): `C_comp` (높게) → 충격 흡수
- 신장 시 (반발): `C_ext` (낮게) → 반동 억제

### 매 스텝 바퀴당 3가지 force 적용

1. **Suspension force** — 수직 지지력. Ray hit point에서 chassis에 적용
...
## 6. CARLA Ray-Wheel 방식과 Genesis 구현 한계

### 6-1. CARLA (Unreal Engine + PhysX) 방식

CARLA는 PhysX 엔진을 백엔드로 사용하며 Ray-Wheel을 엔진 레벨에서 완전히 지원한다.

구조:
  * 차체(chassis): rigid body + collision 있음
  * 바퀴:          시각적 mesh만 (collision 없음)
  * 접지:          바퀴 축에서 -z Ray 발사 → hit_distance 측정
  * 서스펜션:      PhysX 내장 suspension model (spring/damper 자동)
  * 마찰:          PhysX tire force 모델 자동 계산
...
### 6-2. Genesis에서 구현이 안 되는 이유

Genesis는 Ray-Wheel을 엔진 레벨에서 지원하지 않는다.
...
| 방식 | 결과 | 비고 |
|------|------|------|
| Spring force (Step 4) | ❌ 실패 | 관통 후 폭발적 force → 튕김 |
| Constraint 근사 (Step 6) | ✅ 성공 | z_std=0.000, 즉각 수렴 |
| Constraint + Traction (Step 7) | 🔄 진행 중 | 수평력 디버깅 중 |


### Constraint 근사 구현

![](../res/0502/friction.mp4)

https://github.com/i1uvmango/Genesis_ai_graphicstudy/issues/46#issue-4378653519

매 `scene.step()` 후:

1. Lidar로 바퀴 관통 깊이(`max_comp`) 측정
2. 관통 시 `set_pos`로 차체를 `max_comp`만큼 위로 보정
...
- MPPI 2001 env 배치 적용 시 성능 문제 미검증
...
우회 방법:
  set_pos + set_dofs_velocity로 PhysX constraint 근사
  → 정적 안착은 성공
...
- ✅ Genesis Lidar API 작동 확인 (GridPattern -z)
- ✅ Genesis Lidar self-ignore 동작 확인
- ✅ `apply_links_external_force` API 작동 확인
- ✅ Lidar 배치 (n_envs=2001) 지원 확인
- ✅ Genesis raycaster.py 버그 발견 및 패치
- ✅ Constraint 근사 방식으로 Plane terrain 안착 성공
