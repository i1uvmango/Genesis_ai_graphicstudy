...
<!-- TODO: 이 절의 도입 한 줄 — "Pacejka는 slip → friction을 다루고, 여기서는 그 slip을 만드는 throttle/brake → wheel omega 체인을 다룬다" 정도 -->
...
```
T_brake = brake * T_MAX * torch.tanh(omega_t / OMEGA_EPS)    # 4륜 제동
T_fric  = TIRE_RADIUS * F_long_w                             # Pacejka 마찰력

domega  = (T_drive - T_brake - T_fric) / I_WHEEL
```
...
* **가속 중에는 바퀴 회전을 늦추는 방향, 제동 중에는 회전 유지 방향으로 작용**
