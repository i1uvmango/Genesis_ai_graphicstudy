...
| 현실 제약 | ❌ 슬립, 관성, 마찰 무시 | ✅ 슬립, 원심력, 마찰 반영 |
...
매 Frame t 마다:
  ...
  ② 각 환경에 서로 다른 노이즈 扰동(perturbation) 주입
...
| **학습하는 것** | 제어값 간의 스케일·변환 관계 | 물리 상태 → 최적 제어의 인과관계 (Inverse Dynamics) |
| **물리 모델 차이 처리** | ❌ 무시 | ✅ 동역학 state로 명시적 보정 |
| **일반화** | 경로 의존적 | 임의 경로에서도 안정적 |
