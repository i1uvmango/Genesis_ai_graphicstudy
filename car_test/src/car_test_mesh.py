...
def main():
    ...
    # DAE 파일 분석 결과:
    # - 실제 크기: X=3.673m, Y=1.888m, Z=1.578m
    # - 예상 크기: X=2.5m, Y=1.2m, Z=0.5m
    # - 중심점: (0, 0, 1.271) - Z가 너무 높음
    # - 스케일 비율: X=0.68, Y=0.64, Z=0.32 (평균 약 0.55)
    # - DAE 중심점이 1.271m이므로, 차체 하단을 0으로 맞추려면 pos의 Z를 -0.636 정도로 설정
    # - 하지만 collision은 box (2.5 x 1.2 x 0.5)이고 visual만 DAE이므로, collision 기준으로 설정
    # - 차체 중심을 0.8으로 설정하면 차체 하단은 0.8 - 0.25 = 0.55m
    # - DAE visual은 origin xyz="0 0 -0.4"로 조정했으므로 약간 아래로 이동
    car = scene.add_entity(
        morph=gs.morphs.URDF(
            file="./car_dae.urdf", 
            pos=(0, 0, 0.8),
            scale=0.55,  # DAE 파일 크기를 URDF 예상 크기로 맞추기 위한 스케일
